...
export async function sendMessage(prevState: FormState, formData: FormData) {
  ...
  const { fullName, email, localisation, investType, message } =
    validatedFields.data;

  let infoMessage = "";

  const res = await fetch("https://formsubmit.co/ajax/kontakt@kapprojekt.pl", {
    method: "POST",
    headers: {
  ...
  if (response.success) infoMessage = "Wysłano wiadomość.";
  else infoMessage = "Wystąpił błąd podczas wysyłania.";
