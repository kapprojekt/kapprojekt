  	...
  	return (
		<nav className={`z-20 fixed top-0 left-0 right-0 flex justify-between items-center px-10 before:absolute
			...
			<div className='lg:hidden' ref={refMenu}>
				...
				<div className={`absolute lg:hidden rounded-sm flex flex-col items-center inset-x-8 top-20 text-xl
					...
					${isOpen ? 'translate-y-0 scale-100 opacity-100 visible' : '-translate-y-[300px] scale-0 opacity-0 invisible'}`}>
					{links.map((link) => (
						<Link
							key={link.label}
							className='inline-block px-10 my-4 font-semibold drop-shadow-lg' href={link.href}>
								{link.label}
						</Link>))}
				</div>
			</div>
